...
if __name__ == '__main__':
    ...
    path_v = plath_test.park_v()      # 垂直泊车的路径
    ...
    path_v = interpolate_path(path_v, sample_rate=3)

    # env.draw_path(interpolated_path)
    env.draw_path(interpolated_park_path)
    env.draw_car()
    print(interpolated_park_path.shape)     # shape:(60,2)
    print(path_v.shape)
    ...
    for i, point in enumerate(path_v):
        ...
        if my_car.v == 0:
            ...
            # pwm_forward.Motor_Forward(my_car.v * 5)
            print("v > 0")
        else:
            pwm_forward.Motor_back(10)
            # pwm_forward.Motor_back(5 * abs(my_car.v))
            print("v < 0")
