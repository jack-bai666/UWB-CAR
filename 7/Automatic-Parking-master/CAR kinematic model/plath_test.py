...
def vertical_path(xy1, xy2):
    ...
    # 从初始点到A点
    list0 = []
    for y in range(y0, y_a, 12):             # 步长分别为 2,2,4,2
        list0.append((x0, y))
    # list0.reverse()
...
#############################################
    ...
    # 参数方程
    theta_b = math.atan2(118 - 72, 33 + 20)
    theta = np.arange(0, theta_b, 0.16)
    x1 = a + rr * np.cos(theta)
    y1 = b + rr * np.sin(theta)
    ...
    for i in range(len(x1)):
        list1.append((int(x1[i]), int(y1[i])))
        # print((x1[i],y1[i]))
    # print(list1)

    a, b = (86, 164)  # 圆心坐标
    theta_b = math.atan2(164 - 118, 86 - 33)
    # 参数方程
    theta = np.arange(np.pi + theta_b, 3/2 * np.pi, 0.16)
    x2 = a + rr * np.cos(theta)
    y2 = b + rr * np.sin(theta)
    ...
    for i in range(len(x2)):
        list2.append((int(x2[i]), int(y2[i])))

    #############################################
    # list1 = []
    # # list1.append((x_b, y_b))
    # # 从a点到b点
    # for x in range(x_b, x_a, 6):
...
#############################################
    # 从c点到结束点
    list3 = []
    for x in range(x_c+5, x_end + 5, 10):
    ...
    ensure_path2 = []
    for x in range(x_end,x_end +5,3):
        ensure_path2.append((x, y_end))
    ensure_path2.reverse()

    list_all.append(list0 + list1 + list2 + list3 + ensure_path2)  # 不同半径下产生的路径
    list_t = list0 + list1 + list2 + list3 + ensure_path2

    # print(type(list_all[1][1][0]))
    print("chang:", list_t)
    print("chang:",len(list_t))
