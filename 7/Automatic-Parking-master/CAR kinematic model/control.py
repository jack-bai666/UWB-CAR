...
class Car_Dynamics:
    def __init__(self, x_0, y_0, v_0, psi_0, length, dt):
        ...
        self.v = v_0
        self.psi = psi_0        # 航行角
        self.state = np.array([[self.x, self.y, self.v, self.psi]]).T   #转置成6*1
                                # delta 偏转角
...
class MPC_Controller:
    def __init__(self):
        ...
        self.Rd = np.diag([0.01, 1.0])                 # input difference cost matrix   输入差异成本矩阵，用于控制输入变化的代价
    ...
    def optimize(self, my_car, points):
        ...
        bnd = [(-5, 5),(np.deg2rad(-60), np.deg2rad(60))]*self.horiz        # 约束
...
class Linear_MPC_Controller:
    def __init__(self):
        ...
        self.Rd = np.diag([1, 0.001])                 # input difference cost matrix
